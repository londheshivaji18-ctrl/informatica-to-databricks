...
def test_filter_vehicle_policies(spark):
    df = spark.sql("SELECT * FROM VALUES (1, 'vehicle'), (2, 'home'), (3, 'VeHiCle') AS data(policy_number, policy_type)")
    out = T.filter_vehicle_policies(df)
    assert out.count() == 2


def test_join_and_aggregate_fraud_by_state_type(spark):
    ...
    expected = spark.sql("SELECT CAST('CA' AS string) AS incident_state, CAST('Single Vehicle Collision' AS string) AS incident_type, CAST(1 AS bigint) AS Count_Incident")
